...
- 🗑️ **資料刪除申請** - 對於已提交的資料，可發起刪除請求，由管理員審核。且使用者可查看歷史申請紀錄（含已核准與已駁回）。
...
- 🛡️ **刪除請求管理** - 專屬「刪除表單」審核介面：
  - **歷程留存**：完整記錄所有刪除請求（待審核、已核准、已駁回），且審核後資料永久保留供查閱。
  - **即時通知**：導覽列顯示待審核案件數量徽章 (紅色 Badge)。
- 📥 **CSV 批次匯入** - 提供範本下載，支援 Excel 編輯後批次匯入資料。
- 📤 **進階資料匯出** - 支援將系統資料匯出為專用 CSV 格式（多選欄位拆分、Yes/No 轉換為 0/1、MIC 值獨立欄位），便於統計分析。
...
- Express.js
- sql.js (SQLite WASM) - 支援 Render.com 等無原生綁定環境
- bcryptjs (安全性密碼雜湊)
- express-session (Session 狀態管理)

## 部署 (Deployment)

本專案已配置為可直接部署至 [Render.com](https://render.com)。
